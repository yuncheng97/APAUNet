...
from model.TransBTS.Transformer import TransformerModel
from model.TransBTS.PositionalEncoding import FixedPositionalEncoding,LearnedPositionalEncoding
from model.TransBTS.Unet_skipconnection import Unet
...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        self.flatten_dim = 96 * num_channels
        ...
        self.Unet = Unet(in_channels=1, base_channels=12, num_classes=3)
...
class BTS(TransformerBTS):
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        y2 = self.DeUp2(y3, x1_1)  # (1, 16, 96, 96, 96)
        y2 = self.DeBlock2(y2)

        y = self.endconv(y2)      # (1, 4, 96, 96, 96)
        y = self.Softmax(y)
        return y
