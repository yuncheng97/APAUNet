...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm, n=2, ks=3, stride=1, padding=1):
        ...
        if is_batchnorm:
            ...
            for i in range(1, n + 1):
                conv = nn.Sequential(nn.Conv3d(in_size, out_size, ks, s, p), nn.ReLU(inplace=True), )
                setattr(self, 'conv%d' % i, conv)
                in_size = out_size
...
class UNet3Plus(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, trilinear=True, feature_scale=4,
        ...
        self.hd5_UT_hd4 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True)  # 14*14
