...
class Trainer(object):
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            if self.config["model_name"] == "APAUNet" and (epoch + 1) % 10 == 0:
    ...
    def evaluation(self, epoch):
        ...
        print(f"{datetime.now()}\t"f"Epoch: {epoch}\t"f"dice_average: {dice_mean:0.4f}\t"f"dice_organ: {np.mean(dice_organ_list):0.4f}\t"f"dice_cancer: {np.mean(dice_tumor_list):0.3f}\t")
        return dice_mean
    ...
    def save(self, dice_mean, epoch):
        ...
        torch.save(checkpoint, os.path.join(self.config["save_path"], 'best_'+self.config["log"]+self.config["dataset"][7:]+'.pth'))
